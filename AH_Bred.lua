...
function imgui.OnDrawFrame()
	...
	if i_info_update.v then
		...
		imgui.Begin(u8"Обновления", i_info_update)
		imgui.Text(u8'Версия 4.0:')
		imgui.Text(u8"- Добавила авто ввод паролей.")
		...
		imgui.Text(u8"- Добавила авто раздачу виртов и очков за онлайн.")
		imgui.Text(u8"- Добавила автообновление скрипта.")
		imgui.Text(u8"- Фикс некоторых ошибок.")
	...
	if ah_help.v then
		...
		imgui.Text(u8'Флуды')
		imgui.SameLine()
		imgui.TextQuestion(u8'Нажав на кнопку в чат отправится выбранное вами сообщение.')
		if imgui.Button(u8'Репорт') then
		local var1 = "/mess 17 ==========================================================="
			sampSendChat(var1)
			local var2 = "/mess 15 Мешают читеры или нарушители правил? Сообщите админитрации!"
			sampSendChat(var2)
			local var3 = "/mess 15 Жалоба администрации на игрока: /report"
			sampSendChat(var3)
			sampSendChat(var1)
    end
	imgui.SameLine()
	if imgui.Button(u8'Обман') then
sampSendChat('/mess 6 Доброго времени суток уважаемые игроки Russian Drift Server')
sampSendChat('/mess 6 Сегодня я хочу донести до вас такую информацию:')
sampSendChat('/mess 6 Чтобы потом не возникали лишних вопросов, что вас где-то обманули')
sampSendChat('/mess 6 Убедительная просьба все сделки записывать на видео')
sampSendChat('/mess 6 Ведь администрация не сможет вам помочь без вещественных доказательств')
sampSendChat('/mess 6 Примеры программ для записи: Fraps, Bandicam, ShadowPlay,OBS Studio.')
sampSendChat('/mess 6 Приятной игры и хорошего настроения на Russian Drift Server<3')
	end
		imgui.End()
		end
